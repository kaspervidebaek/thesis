...
\paragraph{Running time} From an worst-case running-time perspective, this process does not look good: In the worst case, we will have merge the entire file and hit a conflict on the last line. Then we will start over, parse the files, match the functions, and then try a line merge on each of these matches. Each function will fail on the last line and we will have to do a structure match.


$(r+c) \cdot lm (l_a, l_o, l_b) + c ( ma(f_a, f_o, f_b) +  f \cdot lm(lf_a, lf_o, lf_b) + fc \cdot sm(nf_a, nf_o, nf_b) )$


\begin{itemize}
\item The growth of the line-merge-function $lm$ is smaller than $sm$
\item The number of nodes in L-M
...
\begin{equation}
\begin{split}
(r+c) \cdot lm (l_a, l_o, l_b) + \\
 ( ma(f_a, f_o, f_b) + \\
 		(fc+fr) \cdot lm(lf_a, lf_o, lf_b) +\\
 		fc \cdot sm(nf_a, nf_o, nf_b) ) \cdot c 
\end{split}
\end{equation}

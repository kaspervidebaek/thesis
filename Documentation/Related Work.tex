...
\paragraph{Running time} The running time of Needlemann-Wunch is $O(n \cdot m)$, where $n$ and $m$ is the number of items in the two input sequences.
...
\paragraph{Running time} The running time for the two-way alignment functions are $O(|A||O|)$ and $O(|B||O|)$ the result from these functions are maximally of the size $|A|+|O|$ and $|B|+|O|$ in the case that there exists no matchings. This gives an overall worst-case running-time of $O((|A|+|O|) \cdot (|B|+|O|))$. This case is, however quite unlikely if A and B are actual evolved items of O. 
...
\paragraph{Result} The result chunking is quite interesting.

...
The overall goal of a version control system is to support development of documents, by keeping track of changes over time. In the context of software development, version control systems are frequently used to keep track of source code. Two overall schools of version control systems exists: Distributed and centralized. In the context of this thesis, we do not distinguish between these two - but abstract all the different merging-scenarios possible into the simple concept of merging two branches with a common ancestor.\todo[inline]{Skrive mere om process?}
...
A conflict can be defined in quite a few ways. Merging tools of version control systems are mostly line based and oblivious to the structure of the content of the files contained. They work by sequence matching the two branches with the common ancestor, and given this matching it will detect conflict, or do an automatic merge.

Version control systems can contain any type of file. However, binary files are generally not merged by version control systems. Instead they are treated as conflicts instantly. Other document types - for example plain text files, source code or XML files - are more easily merged by line based tools. They are also quite common in version control systems.

A line based approach to such files often works. Their content is grouped in lines which is structurally independent, and therefore inserting and deleting lines during the merge is quite likely to generate a structurally valid file. 

It is easy to see where it fails: While XML files can be split into a new line per XML-tag, it is not necessary to do so, and 

While it is quite likely that a line based merge tool generates a structurally valid output, that however does not mean that the outputted documents are 

- will have an underlying structure that is either readable by a machine, or by human beings.  



-- Conflicts
-- Merging are approximations - are merge does not guarentee anything (from a line perspective)

...
We will run two-way sequence-matching two times on the sequence-pairs (A, O) and (O, B). Afterwards we will run a two-way-matching on these two results, where equality between two two-way matches is defined as them not having an empty O-item, and having equal O-items. This will in the end giving us a list of matchings. The pseudocode can be seen in Algorithm \ref{ThreeWayMatchingAlgorithm}.

\paragraph{Running time} The running time for the two-way alignment functions are $O(|A||O|t)$ and $O(|B||O|t)$, where $t$ is running time for $equality$. The result from these functions are maximally of the size $|A|+|O|$ and $|B|+|O|$ in the case that there exists no matchings. This gives an overall worst-case running-time of $O((|A|+|O|) \cdot (|B|+|O|) \cdot t)$. This case is, however quite unlikely if A and B are actual evolved items of O. 

\todo{Add equality function time to this}
...
\paragraph{Running time} The running time of the first ThreeWayMatching function is  $O((|A|+|O|) \cdot (|B|+|O|) \cdot t)$ and it returns maximally $A+O+B$ items, which are then passed to the chunking function, which run in $O(n)$ time. This gives a worst case running time of

\begin{equation}
O((|A|+|O|) \cdot (|B|+|O|) \cdot t + (A+B+C)(|A|+|O|) \cdot (|B|+|O|) \cdot t')
\end{equation}

However this is quite an exaggeration. Th situation that this describes if each chunk contains the entire matching. A more accurate way of looking at  would be to desribe the sum:

\begin{equation}
\sum_{c \in C} (|c_a| + |c_o|) (|c_o| + |c_b| \cdot t')
\end{equation}

We should now notice that the following is true:
...
køretid for ThreeWaymatching er (a+o)(o+b) og den retunerer (a+o+b) items.
antallet af chunks vil være mellem 1 og (a+o+b)

den naive analyse siger at vi rammer:
- (a+o)(o+b) + (a+o+b)(a+o)(o+b)
(den ydre threeway-matching + størt mængde chunks * indre three-way matchings værste køretid)

%dog:
%- hvis mængden af chunks er (a+o+b), er køretiden for ThreeWayMatching 1.
%- hvis mængden af chunks er 1, er køretiden for ThreeWayMatching (a+o)(o+b).
%- altså:
%-- antallet er chunks er en en slags omvendt proportionel med køretiden af den indre ThreeWaymatching

%it postulat: køretiden for _hele_ foreach løkken er maksimalt (a+o)(o+b).


(x + a + y + b)(z + c + y + b)
(x + y)(y + z) + (a +b)(b+c)

http://i.imgur.com/USlA8ip.png
http://i.imgur.com/J3VMMhH.png


Den samlede sum af alle ledende $c_a$, $c_o$, $c_b$ vil være $|A|$

\title{Algorithms for merging syntax trees}
...
In this thesis we will investigate the topic of automatic merging source code in version control systems. We will investigate the line-based tools in use today, and the conflicts that naturally occur when merging source code with them. We will look into benefit and shortcomings of changing from such a line-based approach into more structure-awareness; specifically we will look into whether structure awareness give better merge results, and if the overhead in running time that such structure awareness brings is worth the effort.

We will present an algorithmic toolbox for merging an matching, and use this framework to create an algorithm that can do three-way merge of C\# code. We will motivate the algorithm theoretically and do a upper bound analysis of the algorithm.
...
\todo[inline]{Skriv mere om den her fodnote}
...
\todo[inline]{Motivate why these functions are important}
...
In this section we describe the functions used to all nodes from the method level and downwards in the syntax trees. All the functions in this chapter are mutually recursive and because of this we analyse their runnning times at the end of the entire section instead of analysing them individually.
    		...
    		\ForAll{$m$ in $c$}
            	...
            	\If{$m_A$, $m_O$ or $m_B$ has changed since last iteration}
        	        ...
        	        \If{$m_A$, $m_O$ or $m_B$ is not null}
            	        \State add \Call{MergeBlockStart}{$P(m_\texttt{relevant item})$} to $o$
...
\clearpage
\section{Future work}
Several questions are left unanswered at the end of this thesis. In this section I will give a brief overview of these.
...
\todo[inline]{Node that we added "one block down" detection}

\todo[inline]{We have found: \\
- Tree merging is generally not applicable for VCS \\
- Line-based with fallback is, hwoever. \\
-- This creates high variance  \\
- Added Line-based function \\
- Trivial conflicts in tree-based ; we should not resovle all conflict. \\
- Created general three-way set-matching algorithm \\
- Similarity of functions \\
- Created an overall merging algorithm \\
- Running time asymptoticly high, but several factors point towards practically applicable\\

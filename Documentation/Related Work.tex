...
Programming languages are abstractions over the way computers work. They makes it easier for a programmer to express the problems they need to solve, without requiring the programmer to handle intricate details about memory and processors, and they allow the user to write programs as text files, and let the computer handle the process of executing it.
...
The process of turning programs into execution relies on lexers and parsers, generated through a token specification and a context-free grammar. A token in this context means any concrete string of text that is legal in the program. A string could be a token, a number, a keyword and a parenthesis. In an analogy they define any words that a language can consist of. The lexer will simply output a sequence of tokens, to be used by the grammar, in this the list of tokens, only terminals exists as actual the string representation that they have in the original file.

The context free grammar is the structure that defines which tokens are valid at a given point. It defines that only expressions are valid conditions for if-statements, and that only statements are valid in the body. In the analogy, it is the definition of valid sentences in a language.

Given a grammar of a language, we will be able to to parse the syntax of a legal program, or to generate syntax errors for invalid ones. A little token specification and grammar is presented in Figure \ref{token} and Figure \ref{grammar}. It resembles some of the constructs you would see in a imperative programming language, but is a nonsensical language that cannot be used to much besides supporting examples needed in this section.
...
The purpose of parsing source code is often iterating over the elements of it, to generate a syntax tree. A syntax tree is a tree representation of the program. The syntax tree representation is quite good for further processing of the program, however at this point a lot of details about the concrete textual representation of the code has also left behind. And example of a syntax tree can be found in Figure \ref{syntaxtree}. 

Syntactic sugar can for example easily be lost, depending on what kind of choice of syntax tree is done. On the syntax tree level it might for example be easier to convert \texttt{int? x;} into the same structure as \texttt{Nullable<int> x;} to not clutter the abstract syntax tree with unnecessary information.
...
\subsection{The lost information}
- Data outside syntax trees (comments)
- Data that breaks the syntax tree (preprocesser directives)


\subsection{Undecidability}
A last point that is important to consider about merging algorithms, is the properties of a program is generally undecidable without actually running it. Many different strictness levels exists across the landscape of programming languages, but even the strictest programming language will allow incorrect programs through - simply because 
...
In this section I will dive into the implementation of the code. The main function of the algorithm looks like this:

...
- Ordering in classes. \\
- Ordering in parameter and argumet-lists. \\
- Very similar statements where different nodes have been changed in each branch. \\
-- Argue that "small expressions" could be external variables - i.e. it is all about coding style whether a line based conflict will happen. }
...
The running time for the match is $O(n^2 \log n)$, and it returns $O(n)$ matchings, giving an overall running time of the loop $O((n^2 \log n)^n)$ running time. Arguable not a very good 

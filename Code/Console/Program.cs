    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var x = new List<Tuple<string, int>>
            {
                Tuple.Create("a", 1),
                Tuple.Create("a", 2),
                Tuple.Create((string)null, 3),
                Tuple.Create("b", 4),
                Tuple.Create("b", 5),
                Tuple.Create((string)null, 6),
                Tuple.Create((string)null, 9),
                Tuple.Create("a", 7),
            };

            var y = x.ChunkBy(z => z.Item1 == null);

            foreach (var c in y)
            {
                Console.WriteLine("Key: " + c.Key);
                Console.WriteLine("Values: " + String.Join(", ", c.Select(k => k.Item2).ToArray()));
            }

...
In this thesis we will investigate merging algorithms. During a merging process, conflicts will naturally occur if a single part is changed in both branches, into different things. Most widely used merging tools treat code as flat text files, and compare each line of code without considering the structure of the code. 

The purpose of this project is to investigate if it is practical to create merge tools that break concrete syntax into syntax trees and do tree merging on these instead, and if such syntax-aware merging would reduce the amount of conflict in branch-merging.
    ...
    \If{that does not work}
        ...
        \ForAll{chunks}
            ...
            \ElsIf{chunk is secondary equal}
                \State for each line \Return \Call{MergeNode}{A, O, B}

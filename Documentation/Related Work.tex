...
The goal described earlier was to minimize the amount of trivial conflicts found in the syntax tree. But what is a trivial conflict? A trivial conflict is a conflict that cannot be solved in line based tools, but where knowledge of the structure of the language will actually allow us to make mostly informed choices about what is intended by the user.

\todo[inline]{Insert code examples in all of the below.}

\paragraph{Reordering of class-members} Class-members needs to have a definition order in concrete syntax, yet this order has no meaning for the execution in most languages. If one branch moves a method of a class, and another branch makes change to this branch, a line based tool will report a conflict. An intelligent merge of these would be to propagate the changes in one branches into the moved methods body.

Given a syntax tree, we know where method-definitions exists, and we can treat them as a set. This gives us a problem of matching functions together across the three branches; we will need to create some heuristic that allow us to decide whether two functions are the same or not. After that can merge the function bodies of the methods matched from the three branches.

\paragraph{Reorderings in parameter-lists and argument-lists} Parameter-lists and argument-lists are closely tied together. A reordering of the parameters in a method definition in one branch will cause all argument lists to also be reordered. If the second branch adds a parameter at the same time, this will also be added as an argument in all calls to this function. From a line based perspective this will generate conflicts, however given a syntax tree this can be solved by matching parameters and arguments across branches and create new argument- and parameter-lists, that both contain the reorderings and the added items.
...
\end{multicols*}
\caption{Three statements, that will conflict in a line-based merge}
\label{ConflictingLines}

\end{figure}

Given three input lines, that are vaguely similar as in figure \ref{ConflictingLines} where both branches have changed small parts of the behaviour of the line, what should we merge it into? Both branches want to change small parts of behaviour, and as such producing a conflict could easily be seen as the best choice. However a simple re-factoring of the code like in figure \ref{RefactoredForNoConflict} will make a line based merge it without a conflict.

This show the arbitrariness of the line based merge approach; in this case it wont mean much due to it being a small integer expression, but it is easy to imagine more complicated expressions where the same refactoring will make a big difference. 
...
So what do we do? If we can match nodes in the syntax tree of the branches when they are similar, then we can do a top-down merge of nodes; only if the structure or content of two sub-nodes has changed different structure or content, we will have to generate a conflict. This allows us to actively merge the branches in figure \ref{ConflictingLines} into $a \gets 1+2+4$ without user interaction.

\paragraph{Renaming identifiers} Local variables and class members have an identifier that is used to reference them from other places in the code. This identifier can be renamed. Renaming an identifier will also have the consequence of changing the identifier all the places where this specific identifier is referenced. If a rename happens in one branch, and somebody changes or adds a line that references this identifier in another branch, the merge will either give a conflict or a compile error: 

\begin{itemize}
\item If somebody changes an expression around an identifier that has been renamed in another branch this would generate a conflict in a line based approach. This might not actually produce a conflict in our approach, since we only modify parts of the sub-tree, but it might still do it.
\item If a identifier has been renamed in one branched, while somebody has added a new reference to it, in the opposite branch, this will generate merge that gives an compile error.
\end{itemize}
...
The process of reordering can be seen in Figure \ref{Reordering}. This figure also shows that the ambiguity in our two-way-matching algorithm can lead to different potential outcomes of the reordering algorithm; if the $A \rightarrow O$-matching chooses to match the 4; implying that 4 has only moved in the $O \rightarrow B$-match, we will produce a conflict-free output. If the $A \rightarrow O$-matching chooses 3 as the match, we will get a conflict where 4 is being inserted both places. The safe bet would of course to try always to modify the algorithm such that a conflict always would be produced, and this would probably be possible to do, by modifying the two-way-matching to return all possible matchings, and let the three-way-matching algorithm choose a pair that share most matchings.

\paragraph{Running time} ThreeWayMatching runs in $O(|A||O| t + |B||O| t + |A|+|O|+|B| )$ time, and in worst case it returns $|A|+|O|+|B|$ items. The loop in the algorithm only has constant time operations which means the overall running time will be $O(|A|+|O|+|B|)$ for the loop. Conflict detection can be done by keeping a hash set on the side of the results already added which will give us constant time lookup for each item; leaving us with $O(|A|+|O|+|B|)$ 
time.
...
\paragraph{Running time} Three-way matching and chunking gives us an overall running time of $O(|A||O| t + |B||O| t + |A|+|O|+|B|)$, given that $t$ is the running time of $equality$. The loop will run $|A|+|O|+|B|$ times. All operations inside the loop are constant time, except for the conflict-handler which has an unknown running time. The above two use cases can also be divided into two scenarios for the conflict-handler:

\begin{itemize}
	\item When the conflict-handler output the chunks as conflicting it will only run constant time operations on the chunks, for any chunk. The running time will be:\\
		\begin{equation}
			O(|A||O| t + |B||O| t + |A|+|O|+|B| ) \nonumber
		\end{equation}

	
	\item When the conflict-handler flushes the output and starts a new merging algorithm it will also break the loop, meaning it will only one time. Given that $t'$ is the time it takes to run the conflicthandler, the running time will be:\\
		\begin{equation}
...
\paragraph{Running time} The running time for this is the second scenario of the sequence based merging algorithm, with the equality function being constant time. The running time for $t'$ will be defined later. 
		\begin{equation}
			O(|A||O| + |B||O| + |A|+|O|+|B| + t') \nonumber
		\end{equation}
...
We do an unordered match between the the base file, and the two input brances using heuristics to determine the match of the input trees children. Nodes that are matched in the syntax trees are merged together. The overall process will be:
...
\paragraph{Running time} $an(x)$ is the total amount of nodes that is found in the tree from the root, down to until the leaves that are class member; it will contain all root-items, namespaces, classes and class members, but no nodes from below class-members. $n$ is $max((an(A) an(O), an(B))$ and the time for running the cost function is constant. 

The running time for the match is $O(n^2 \log n)$, and it returns $O(n)$ matchings, giving an overall running time of the loop $O((n^2 \log n)^n)$ running time. 
...
\todo[inline]{This subsubsection belongs in this subsection, but is not written for it yet, and is not finished}
...
\todo[inline]{Below here is the actual start of this "Mergin Functions" subsection}

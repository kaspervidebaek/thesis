...
This approach will most definitely be slower than running a line based merging algorithm, however it will hopefully be faster than the current structured merging approaches, and thereby more usable in a version control scenario.
...
Bipartite matching with costs can be solved in cubic time, using flow-graphs. Our overall approach for matching the functions sets $(l_1, l_2, ..., l_{size(l)})$, $(b_1, b_2, ..., b_{size(b)})$ and $(r_1, r_2, ..., r_{size(r)})$ will be to do a bipartite matching between the pairs $(b, l)$ and $(b, r)$.

There will be no guarantee that the different function sets are equal in size, or even that a similarity function will produce an edge between two nodes. 

Therefore we need to modify bipartite cost matching a little. Given the function that takes two equal size sets (xs, ys) and returns a matching of all the elements.

\todo[inline]{Write about aux. nodes}

The end condition will be modified to when there exists no path between source and sink. All remaining items not in the matching will generate matching of either $(x, \epsilon)$ or $(\epsilon, y)$ and that entire matching will be returned.

Given these two matchings, we will do another unordered matching between the outputted matching, where will match on the $b$-items being equal.
...
The end result of the tree matching algorithm will be a set of merged class members. This set needs to be serialized into a concrete file consisting of actual syntax. A problem arisen from that, is what order to output class members in.

...
\Function{MergeFile}{A, O, B}
   \State $\texttt{conflicthandler} \gets (output, chunk) \Rightarrow$
      \State ~~~~~~~~~~~~~~~~~~~~ clear $output$
      \State ~~~~~~~~~~~~~~~~~~~~ let $output$ be the result of \Call{MergeSyntax}{A, O, B}
      \State ~~~~~~~~~~~~~~~~~~~~ terminate outer merge function
	\State $\texttt{equality} \gets (x, y) \Rightarrow x = y$
	\State \Return \Call{Merge}{A, O, B, \texttt{equality}, \texttt{conflicthandler}}
\EndFunction
\end{algorithmic}
...
\Function{NodeSimilarity}{\texttt{x}, \texttt{y}}
	...
	\ElsIf{\texttt{x} and \texttt{y} are blocks}
		...
		\State \texttt{cs} $\gets$ \Call{Chunking}{m}
		\State \texttt{s} $\gets$ 0, \texttt{t} $\gets$ 0
		\ForAll{\texttt{c} in \texttt{cs}}
			\If{\texttt{c} is stable}
...
\subsection{Merging}
\todo[inline]{Explain that running times are at the end of the section}
...
In this section we will give a brief overview of the overall running time of the algorithm we have created. We generally view $n$ as $O(max(|A|, |O|, |B|)$ or $O(max(|X|, |Y|)$  or $O(|M|)$ depending on the input parameters. $t$ and $t'$ are the running times of input functions in order.

\begin{tabular}{l | l }
Algorithm & Upper bound \\ \hline
TwoWayMatching(X, Y) & $O(n^2 \cdot t)$ \\
ThreeWayMatching(A, O, B) & $O(n^2 \cdot t)$ \\
Chunking(M) & $O(n)$ \\
PriorityChunking(A, O, B, eq, sim) & $O(n^2(t + t')$ \\
TwoWayCostMatching(X, Y, t) & $O(n^2 \log n)$ \\
Three-way set matching(A, O, B) & $O(n^2 \log n)$ \\
Reordering(A, O, B) & $O(n^2)$ \\

Equality(X, Y)  & $O(n)$ \\
Similar(X, Y) & $O(n^4)$ \\

Merge(A, O, B, eq, ch) & $O(n^2 \cdot t)$  or  $O(n^2 \cdot t +  t')$ \\
MergeFile(A, O, B) & $O(n^2 + c)$  \\
MergeSyntax(A, O, B) & $O(n^3 \log n)$ \\
MergeNode(A, O, B) & $O(n^3 + n^6)$ \\
\end{tabular}

The main entry point for our algorithm is \texttt{MergeFile}. It is inputted sequences.
...
\todo[inline]{Things we did not do: \\
- Evaluate the practical running time of the algorithm \\
- Evaluate the actual output of the algorithm \\
- Struktur-matcher og merger \\
- Renaming \\
- Comments og preprocesser \\
- Better three-way matching for reordering? \\
- Using costs of needlemann-wunchs to create better diffs?
}

...
Given a grammar of a language, we will be able to to parse the syntax of a legal program, or to generate syntax errors for invalid ones. A little toy token specification and grammar is presented below. It resembles some of the constructs you would see in a imperative programming language, but is a nonsensical language that cannot be used to much besides exemplifying different constructs.\\

\textbf{Token specification}
\begin{verbatim}
TOKEN:
    IDENTIFIER
    KEYWORD
...
Parsing languages often result in the generation of a syntax tree. A syntax tree is the a tree representing of the program. The syntax tree representation is quite good for further processing of the program, however at this point a lot of details about the concrete textual representation of the code has also left behind.

A abstract syntax tree for the above grammar could look like this:
\begin{verbatim}
program:
  body(statement list)
...
%this also means, that a program like this:
%
%\begin{verbatim}
%true;
%while(false)
...
%\begin{verbatim}
%program([expr(expression(false);while(
%
%\end{verbatim}



Much syntactic sugar is for example lost. Depending on your syntax tree, however it might also be 
...
- Grammars
- Syntax Trees
- Throwing away information (while-for)
